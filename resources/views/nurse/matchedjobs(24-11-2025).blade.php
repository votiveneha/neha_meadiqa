...
<script type="text/javascript">
    ...
    function update_sector_preferences() {
      var isValid = true;
      ...
      if (isValid == true) {
        $.ajax({
        url: "{{ route('nurse.updateSectorPreferences') }}",
        type: "POST",
        cache: false,
        contentType: false,
        processData: false,
        data: new FormData($('#sector_preferences_form')[0]),
        dataType: 'json',
        beforeSend: function() {
          $('#submitSectorPreferences').prop('disabled', true);
          $('#submitSectorPreferences').text('Process....');
        },
        success: function(res) {
          if (res.status == '1') {
            Swal.fire({
              icon: 'success',
              title: 'Success',
              text: 'Sector Preferences Updated Successfully',
            }).then(function() {
              window.location.href = "{{ route('nurse.sector_preferences') }}?page=sector_preferences";
            });
          } else {
            Swal.fire({
              icon: 'error',
        ...
        error: function(errorss) {
          $('#submitSectorPreferences').prop('disabled', false);
          $('#submitSectorPreferences').text('Save Changes');
